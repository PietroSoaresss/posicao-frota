...
public interface TrackingPositionRepository extends JpaRepository<TrackingPosition, Long> {
    ...
    Optional<TrackingPosition> findTopByLicensePlateStartingWithOrderByPacketDateUtcDesc(String licensePlate);

    List<TrackingPosition> findByLicensePlateStartingWithAndPacketDateUtcBetweenOrderByPacketDateUtcAsc(
            String licensePlate,
            Instant from,
            Instant to
    );
