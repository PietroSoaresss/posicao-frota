...
class RateLimitFilterTest {
    ...
    /**
     * When the filter does NOT trust X-Forwarded-For (default), every request
     * is keyed by remoteAddr — so an attacker rotating the header cannot dodge
     * the limit by spoofing distinct fake client IPs.
     */
    @Test
    void ignoresForwardedForHeaderByDefault() throws Exception {
        ...
        MockHttpServletResponse firstSpoof = perform(filter, "203.0.113.10");
        MockHttpServletResponse secondSpoof = perform(filter, "203.0.113.20");

        assertEquals(HttpServletResponse.SC_OK, firstSpoof.getStatus());
        assertEquals(429, secondSpoof.getStatus());
    }

    /**
     * When explicitly trusting the proxy header (behind an ALB/Nginx that
     * rewrites X-Forwarded-For), distinct client IPs are tracked independently.
     */
    @Test
    void tracksClientsIndependentlyByForwardedIpWhenProxyTrusted() throws Exception {
        RateLimitFilter filter = new RateLimitFilter(1, Duration.ofMinutes(1), true);
