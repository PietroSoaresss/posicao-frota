...
class SmokeTest {
    ...
    void testTripCreateRejectsIncompletePayload() throws Exception {
        mvc.perform(post("/trips")
            ...
            .andExpect(jsonPath("$.errors.horse").exists());
    }

    @Test
    void testTripCreateAllowsEmptyOrigins() throws Exception {
        City city = createCity();
        Manufacturer manufacturer = createManufacturer();
        VehicleModel vehicleModel = createVehicleModel(manufacturer);
        Vehicle horse = createVehicle(vehicleModel, "ABC1D23", "cavalo");
        ...
        mvc.perform(post("/trips")
                ...
                .content("""
                    ...
                    """.formatted(driver.getId(), horse.getId(), trailer.getId())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.origins").isArray())
            .andExpect(jsonPath("$.origins").isEmpty())
            .andExpect(jsonPath("$.originLocation").value("Ibiruba/RS"));
    }

    @Test
    void testTripCreateAllowsMissingFreightValue() throws Exception {
        City city = createCity();
        Manufacturer manufacturer = createManufacturer();
        VehicleModel vehicleModel = createVehicleModel(manufacturer);
        Vehicle horse = createVehicle(vehicleModel, "ABC1D23", "cavalo");
        ...
        mvc.perform(post("/trips")
                ...
                .content("""
                    ...
                    """.formatted(driver.getId(), horse.getId(), trailer.getId())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.freightValue").value(0));
    }
    ...
    void testTrackingActiveTripsReturnsLatestHorsePosition() throws Exception {
        ...
        Manufacturer manufacturer = createManufacturer();
        VehicleModel vehicleModel = createVehicleModel(manufacturer);
        Vehicle horse = createVehicle(vehicleModel, "TQV2G10", "cavalo");
        Vehicle trailer = createVehicle(vehicleModel, "XYZ9Z88", "carreta");
        Company company = createCompany(city);
        Driver driver = createDriver(city);
        ...
        position.setLicensePlate("TQV2G102");
        ...
        mvc.perform(get("/tracking/active-trips"))
            ...
            .andExpect(jsonPath("$[0].horsePlate").value("TQV2G10"))
    ...
    @Test
    void testTrackingActiveTripsDeduplicatesSameHorseAfterFinalizingPosition() throws Exception {
        City city = createCity();
        Manufacturer manufacturer = createManufacturer();
        VehicleModel vehicleModel = createVehicleModel(manufacturer);
        Vehicle horse = createVehicle(vehicleModel, "ABC1D23", "cavalo");
        ...
        Long newestTripId = tripRepository.findAll().stream()
                .mapToLong(trip -> trip.getId())
                .max()
                .orElseThrow();

        TrackingPosition position = new TrackingPosition();
        position.setPacketId(9002L);
        position.setSascarVehicleId(123L);
        ...
        mvc.perform(get("/tracking/active-trips"))
            ...
            .andExpect(jsonPath("$[0].tripId").value(newestTripId))
            .andExpect(jsonPath("$[0].horsePlate").value("ABC1D23"))
            .andExpect(jsonPath("$[0].status").value("Vazio"))
            .andExpect(jsonPath("$[0].latestPosition.packetId").value(9002));
    }
